...
public:
    DivineBlessingSkill() : Skill("Divine Blessing", "Restores 15 HP to all allies on the map.", 15) {}
...
public:
    PurificationSkill() : Skill("Purification", "Damages enemies in a line. Instantly kills Undead.", 20) {}
    bool execute(Character& caster, GameState& state) override {
        int dx[] = { 0, 0, -1, 1 };
        int dy[] = { -1, 1, 0, 0 };
        ...
        for (auto target : bestTargets) {
            if (target->isUndead()) {
                ...
                if (target->resistsPurification()) {
                    target->takeSkillDamage(caster.atk * 1.5, target->x, target->y);
...
public:
    DivineProtectionSkill() : Skill("Divine Protection", "Grants a 10 HP shield to the most injured ally.", 15) {}
    bool execute(Character& caster, GameState& state) override {
