...
std::vector<std::string> GameUI::renderInterfaceLines(const GameState& state) const {
    ...
    auto getEnemyStr = [&](int i) -> std::string {
        if (i >= 0 && i < (int)enemies.size()) {
            ...
            return e->name + " " + (e->isAlive() ? "HP:" + std::to_string(e->hp) : "Dead");
        }
        return "";
    };
    ...
    for (int i = 0; i < maxRows; ++i) {
        ...
        if (i < (int)allies.size()) {
            ...
            leftContent = prefix + a->name + ": " + status;
        }
...
std::vector<std::string> GameUI::renderCommandLines(const GameState& state) const {
    ...
    lns.push_back(sep);
    lns.push_back(" [SELECT] 1:K 2:A 3:P 4:M | [MOVE] w,a,s,d | [ATK] f<dir>");
    if (state.getIsSkillMenuOpen()) {
        lns.push_back(" [SKILL] 1~3: Select Skill | [CANCEL] b - Cancel Skill");
    } else {
        lns.push_back(" [SKILL] r - Use Skill | [CANCEL] b - Cancel | [TURN END] z");
    }
