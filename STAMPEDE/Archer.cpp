...
public:
    SwiftMoveSkill() : Skill("신속 이동", "최대 3칸 이동 후엔 제자리에서 공격만 가능.", 1, false) {}
    bool execute(Character& caster, GameState& state) override {
        caster.freeMoveCells = 3;
        caster.swiftMoveActive = true;
        playSfx("quickness.wav"); // 신속 이동 효과음
...
bool Archer::performAttack(char dir, GameState& state) {
    ...
    if (found && targetEnemy) {
        ...
        int hpBefore = targetEnemy->hp; // 실제 적용된 피해 계산용

        for (int i = 0; i < hits; ++i) {
            if (targetEnemy->isAlive()) {
                if (targetEnemy->takeDamage(finalAtk, targetX, targetY)) isWeakHit = true;
                playHitSfx();
            }
        }
        // 표시 데미지는 실제로 깎인 체력(방어력·약점·분열 등 모두 반영)으로
        int totalDmg = hpBefore - targetEnemy->hp;
