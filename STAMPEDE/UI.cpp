...
static std::string heroColor(CharacterClass cls) {
    ...
    case CharacterClass::PRIEST: return "\033[0;38;2;204;51;0m"; // #CC3300
...
static std::string enemyColor(CharacterClass cls) {
    ...
    case CharacterClass::BOSS_DRAGON:      return "\033[0;38;2;153;102;0m"; // #996600
    default: return "";
    }
}
...
std::vector<std::string> GameUI::renderInterfaceLines(const GameState& state) const {
    ...
    if (curIdx == -1) {
        ...
        std::string hpLine = " 체력  : " + std::to_string(sel->hp) + " / " + std::to_string(sel->maxHp);
        if (sel->shieldHp > 0)
            hpLine += "  \033[0;36m(보호막 " + std::to_string(sel->shieldHp) + ")\033[0m";
        lns.push_back(hpLine);
        lns.push_back(" 공격력: " + std::to_string(sel->atk) + "  방어력: " + std::to_string(sel->def) + "  사거리: " + std::to_string(sel->atkRange));
    }
    ...
    auto getEnemyStr = [&](int i) -> std::string {
        if (i >= 0 && i < (int)enemies.size()) {
            ...
            std::string st;
            if (e->isAlive() && e->isStunned) st += " \033[1;90m(기절)\033[0m";
            if (e->isAlive() && e->isShocked) st += " \033[0;93m(감전)\033[0m";
            return "(" + col + icon + "\033[0m" + ") " + col + e->name + "\033[0m" + " " + info + st;
        }
        return "";
    };
    ...
    for (int i = 0; i < maxRows; ++i) {
        ...
        if (i < (int)allies.size()) {
            ...
            // 상태 태그 (우선순위: 기절 > 매혹 > 보호막)
            std::string tagPlain, tagColored;
            if (a->isAlive() && a->isStunned)      { tagPlain = " (기절)"; tagColored = " \033[1;90m(기절)\033[0m"; }
            else if (a->isAlive() && a->charmedBy) { tagPlain = " (매혹)"; tagColored = " \033[1;35m(매혹)\033[0m"; }
            else if (a->shieldHp > 0)              { tagPlain = " (+" + std::to_string(a->shieldHp) + ")"; tagColored = " \033[0;36m" + tagPlain.substr(1) + "\033[0m"; }
            std::string plain   = prefix + "(" + icon + ") " + a->name + ": " + status + tagPlain;
            std::string colored = prefix + "(" + col + icon + "\033[0m" + ") " + col + a->name + "\033[0m" + ": " + status + tagColored;
...
std::vector<std::string> GameUI::renderCommandLines(const GameState& state) const {
    ...
    // 선택한 유닛이 보물상자에 인접하면 개봉 안내 표시 (진한 주황, 굵게)
    if (!state.getIsSkillMenuOpen() && state.selectedAllyNearTreasure()) {
        lns.push_back(" \033[1;38;2;204;85;0m▶ 보물상자가 인접해 있다! [e] 키로 개봉 (AP 20 소비)\033[0m");
    }
