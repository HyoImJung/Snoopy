...
std::vector<std::string> GameUI::renderInterfaceLines(const GameState& state) const {
    ...
    auto getEnemyStr = [&](int i) -> std::string {
        if (i >= 0 && i < (int)enemies.size()) {
            ...
            std::string shockTag = (e->isAlive() && e->isShocked) ? " \033[0;93m(감전)\033[0m" : "";
            return "(" + col + icon + "\033[0m" + ") " + col + e->name + "\033[0m" + " " + info + shockTag;
        }
        return "";
    };
    ...
    for (int i = 0; i < maxRows; ++i) {
        ...
        if (i < (int)allies.size()) {
            ...
            std::string shieldTag = (a->shieldHp > 0) ? " (+" + std::to_string(a->shieldHp) + ")" : "";
            std::string plain   = prefix + "(" + icon + ") " + a->name + ": " + status + shieldTag;
            std::string colored = prefix + "(" + col + icon + "\033[0m" + ") " + col + a->name + "\033[0m" + ": " + status
                                + (shieldTag.empty() ? "" : "\033[0;36m" + shieldTag + "\033[0m");
