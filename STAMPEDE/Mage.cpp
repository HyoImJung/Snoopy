...
public:
    ElectricSkill() : Skill("전격", "타겟과 인접 적에게 1 피해 + 감전 (이동 불가).", 15) {}
    bool execute(Character& caster, GameState& state) override {
        ...
        if (!mainTarget) {
            state.setLastMessage("전격을 쓸 사거리 내 적이 없다!");
            return false;
        }

        state.setLastMessage(caster.name + "이(가) 전격을 시전했다!");
        int tx = mainTarget->x;
        int ty = mainTarget->y;
        ...
        for (auto e : state.getEnemies()) {
            if (e->isAlive()) {
                ...
                if (distToTarget <= 1) {
                    ...
                    state.addHitEffect(e->x, e->y, 1);
                    if (e->applyShock()) {
                        state.setLastMessage(e->name + "이(가) 감전되었다!");
                    }
                    playExplodeSfx(); // 폭발음(explode 1~2 무작위)
                }
            }
        }
        return true;
...
public:
    TeleportSkill() : Skill("텔레포트", "체력 최저 아군을 법사 옆으로 소환. (시전자 탈진)", 20) {}
    bool execute(Character& caster, GameState& state) override {
        ...
        if (!targetAlly) {
            state.setLastMessage("텔레포트시킬 다른 아군이 없다!");
            return false;
        }
        ...
        if (found) {
            ...
            playSfx("teleporter.wav"); // 텔레포트 효과음
            state.setLastMessage(caster.name + "이(가) " + targetAlly->name + "을(를) 소환하고 탈진했다!");
            return true; // 맵 동기화는 GameState가 처리함
        }
        else {
            state.setLastMessage("법사 주변에 텔레포트할 빈 공간이 없다!");
            return false;
        }
    }
};
...
public:
    DestructionRaySkill() : Skill("파괴광선", "직선 방향 모든 적에게 대규모 피해를 입힌다.", 30) {}
    bool execute(Character& caster, GameState& state) override {
        int dx[] = { 0, 0, -1, 1 };
        int dy[] = { -1, 1, 0, 0 };
        ...
        if (bestTargets.empty()) {
            state.setLastMessage("파괴광선을 쏠 일직선상의 적이 없다!");
            return false;
        }

        state.setLastMessage(caster.name + "이(가) 파괴광선을 발사했다!");
...
Mage::Mage(int px, int py)
    : Character("법사", CharacterClass::MAGE, px, py, 35, 18, 2, 4, true)
{
    ...
    if (found) {
        ...
        for (auto e : state.getEnemies()) {
            ...
            if (isDirectHit) {
                ...
                state.addHitEffect(targetX, targetY, atk);
                hitAny = true;
            }
            else if (isSplashHit) {
                ...
                state.addHitEffect(splashX, splashY, static_cast<int>(atk * 0.5));
                hitAny = true;
            }
        }
        ...
        // 결과 메시지 출력
        if (weakHitAny) {
            state.setLastMessage("법사의 파이어볼이 (" + std::to_string(targetX) + ", " + std::to_string(targetY) + ")의 약점에 작렬했다!");
        }
        else {
            state.setLastMessage("법사의 파이어볼이 (" + std::to_string(targetX) + ", " + std::to_string(targetY) + ")에서 폭발했다!");
        }
