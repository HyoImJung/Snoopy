...
public:
    ...
    bool execute(Character& caster, GameState& state) override {
        state.setLastMessage(caster.name + "이(가) 신의 축복을 시전했다! 모든 아군이 회복된다.");
...
public:
    ...
    bool execute(Character& caster, GameState& state) override {
        ...
        if (bestTargets.empty()) {
            state.setLastMessage("정화를 쓸 일직선상의 적이 없다!");
            return false;
        }

        state.setLastMessage(caster.name + "이(가) 신성한 정화를 시전했다!");
        for (auto target : bestTargets) {
            if (target->isUndead()) {
                ...
                if (target->resistsPurification()) {
                    ...
                    state.setLastMessage("신성한 빛이 " + target->name + "을(를) 강타했지만 즉사에 저항했다!");
                }
                else {
                    state.addHitEffect(target->x, target->y, target->hp);
                    target->hp = 0;
                    state.setLastMessage("신성한 빛이 언데드 " + target->name + "을(를) 즉시 소멸시켰다!");
                }
...
public:
    ...
    bool execute(Character& caster, GameState& state) override {
        ...
        if (targetAlly) {
            ...
            playSfx("shield.wav"); // 보호막 효과음
            state.setLastMessage(caster.name + "이(가) " + targetAlly->name + "에게 신의 가호를 내렸다! (보호막 10)");
            return true;
        }
        return false;
    }
...
Priest::Priest(int px, int py)
    : Character("사제", CharacterClass::PRIEST, px, py, 45, 10, 1, 1, true)
{
...
bool Priest::performAttack(char dir, GameState& state) {
    ...
    for (auto a : state.getAllies()) {
        if (a->isAlive() && a->x == tx && a->y == ty) {
            if (a->hp >= a->maxHp) {
                state.setLastMessage(a->name + "은(는) 이미 체력이 가득하다!");
                return false;
            }
            int healAmt = 15; // 회복량
            ...
            state.setLastMessage(name + "이(가) " + a->name + "을(를) " + std::to_string(healAmt) + " 회복시켰다!");
            return true;
        }
    }
    ...
    for (auto e : state.getEnemies()) {
        ...
        if (e->isAlive() && e->isOccupying(tx, ty)) {
            ...
            if (isWeakHit) {
                state.setLastMessage(name + "이(가) " + e->name + "의 약점을 응징했다!");
            }
            else {
                state.setLastMessage(name + "이(가) " + e->name + "을(를) 응징했다!");
            }
            return true;
        }
    }
