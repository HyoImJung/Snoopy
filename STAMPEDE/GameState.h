...
private:
    ...
    std::vector<std::pair<int,int>> treasures; // 보물상자 위치 목록
    ...
    void spawnTreasure();           // 웨이브마다 빈 칸에 보물상자 1개 배치
...
public:
    ...
    // 보물상자: 선택한 아군 인접 칸의 상자를 열어 무작위 버프 적용
    bool tryOpenTreasure(int idx);
    bool isTreasureAt(int x, int y) const;
    bool selectedAllyNearTreasure() const; // 선택 유닛이 상자에 인접했는지
    const std::vector<std::pair<int,int>>& getTreasures() const { return treasures; }
    std::vector<std::pair<int,int>>& getTreasures_mutable() { return treasures; } // 세이브/로드용
