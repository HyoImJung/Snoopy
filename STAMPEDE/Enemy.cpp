...
// Slime 구현
Slime::Slime(int px, int py) : Character("슬라임", CharacterClass::ENEMY_SLIME, px, py, 20, 2, 1, 1, false), hasSplit(false), splitPending(false) {}
...
bool Slime::takeDamage(int dmg, int tx, int ty) {
    ...
    if (hp <= 0 && !hasSplit) {
        hp = maxHp / 2;
        hasSplit = true;
        splitPending = true; // GameState에서 옆 칸에 새 슬라임 소환 예약
    }
    return isDead;
}

// Goblin 구현
Goblin::Goblin(int px, int py) : Character("고블린", CharacterClass::ENEMY_GOBLIN, px, py, 7, 7, 1, 1, false) {}
bool Goblin::performAttack(char dir, GameState& state) { return true; }

// Orc 구현
Orc::Orc(int px, int py) : Character("오크", CharacterClass::ENEMY_ORC, px, py, 30, 5, 1, 1, false) {}
bool Orc::performAttack(char dir, GameState& state) { return false; }

// Undead 구현
Undead::Undead(int px, int py) : Character("언데드", CharacterClass::ENEMY_UNDEAD, px, py, 20, 5, 1, 1, false) {}
bool Undead::performAttack(char dir, GameState& state) { return false; }

// Demon 구현
Demon::Demon(int px, int py) : Character("악마", CharacterClass::ENEMY_DEMON, px, py, 30, 10, 1, 1, false) {}
...
void EnemyManager::processAITurn(std::vector<Character*>& enemies, GameState& state) {
    ...
    // 소환(리치킹 등)으로 enemies가 커져도 이번 턴엔 "기존" 적만 처리한다.
    // (새로 추가된 적까지 돌면 epos 범위를 벗어나 크래시한다)
    ...
    for (int ei = 0; ei < enemyCount; ++ei) {
                ...
                for (auto a : state.getAllies()) {
                    ...
                    if (a->isAlive() && state.manhattanDist(e->x, e->y, a->x, a->y) <= 3) {
                        ...
                        if (a->applyCharm(demon)) {
                            state.setLastMessage(demon->name + "이(가) " + a->name + "을(를) 매혹했다!");
                            charmed = true;
                        }
                    }
                }
        ...
        if (e->cls == CharacterClass::ENEMY_ORC) {
            if (state.manhattanDist(e->x, e->y, TWX, TWY) == 1) {
                ...
                playHitSfx();
                state.setLastMessage(e->name + "이(가) 기지를 맹렬히 공격했다!");
                acted = true;
            }
            destX = TWX; destY = TWY;
        }
            ...
            for (auto a : state.getAllies()) {
                if (a->isAlive() && state.manhattanDist(e->x, e->y, a->x, a->y) == 1) {
                    ...
                    int finalAtk = e->atk;
                    // [패시브] 언데드 - 크리티컬
                    if (e->cls == CharacterClass::ENEMY_UNDEAD && (rand() % 100 < 35)) {
                        finalAtk *= 2;
                        state.setLastMessage("치명타! " + e->name + "이(가) " + a->name + "을(를) 강타했다!");
                    }
                    else {
                        state.setLastMessage(e->name + "이(가) " + a->name + "을(를) 공격했다!");
                    }

                    a->takeDamage(finalAtk);
                    state.addHitEffect(a->x, a->y, finalAtk);
                    playHitSfx();
                    acted = true;
                    break;
                }
            ...
            if (!acted && !e->tauntedBy && state.manhattanDist(e->x, e->y, TWX, TWY) == 1) {
                state.damageTower(e->atk);
                playHitSfx();
                state.setLastMessage(e->name + "이(가) 기지를 공격했다!");
                acted = true;
            }
