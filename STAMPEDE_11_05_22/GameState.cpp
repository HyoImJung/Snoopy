...
bool GameState::tryMoveAlly(int idx, int dx, int dy) {
    ...
    TileType dest = map.getTileAt(nx, ny);
    if (dest != TileType::EMPTY) {
        lastMessage = a->name + " can't move there!";
        return false;
    }
...
void GameState::checkWaveClear() {
    ...
    if (!anyEnemy) {
        ...
        int clearedWave = wave;
        wave++;
        lastMessage = "Wave " + std::to_string(clearedWave) + " Clear! New enemies spawn.";

        // Build list of cells that must not become terrain:
        // ally positions + base cell + enemy spawn row (y=0)
        std::vector<std::pair<int,int> > occupied;
        for (int i = 0; i < (int)allies.size(); i++) {
            if (allies[i]->isAlive())
                occupied.push_back(std::make_pair(allies[i]->x, allies[i]->y));
        }
        occupied.push_back(std::make_pair(7, 14));

        // Randomize terrain for the new wave (harder with each wave)
